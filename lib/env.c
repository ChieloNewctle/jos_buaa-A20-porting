...
struct Env_list env_sched_list;      // Runnable list
...
u_int mkenvid(struct Env *e)
{
	...
	return (idx << ASIDSHIFT) | ((++next_env_id) & ASIDMASK);
}
...
env_alloc(struct Env **new, u_int parent_id)
{
    ...
    e->env_tf.ori_cpsr = (get_cpsr() & ~ARMV7_MODE_MASK | ARMV7_USR_MODE & ~ARMV7_IRQ_MASK) & 0xFFFF;
	e->env_tf.usr_sp = USTACKTOP;
...
void env_check()
{
    ...
    assert(pe0->env_id == 1);
	assert(pe1->env_id == 258);
    assert(pe2->env_id == 515);
