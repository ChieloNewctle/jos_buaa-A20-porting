...
diskaddr(u_int blockno)
{
	if(super && blockno >= super->s_nblocks) {
		panic("blockno %d exceeded nblocks %d\n", blockno, super->s_nblocks);
	}
...
read_super(void)
{
	...
	if (super->s_magic != FS_MAGIC) {
		panic("bad file system magic number %x %x", super->s_magic, FS_MAGIC);
	}
...
//	For each block i, assert(!block_is_free(i))).Check that they're all marked as inuse 
void
read_bitmap(void)
{
	u_int i;
