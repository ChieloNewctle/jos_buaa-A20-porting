...
fork(void)
{
	...
	for(i = 0; i <= PDX(UTOP); ++i) {
		if((*vpd)[i] & PTE_V) {
			u_int k;
			for(j = 0; j < PTE2PT && (k = ((i << PDSHIFT) | (j << PGSHIFT))) < UTOP - 2 * BY2PG; ++j) {
				if((*vpt)[VPN(k)] & PTE_V) {
					duppage(newenvid, VPN(k));
				}
			}
		}
	}
	...
	if(syscall_set_env_status(newenvid, ENV_RUNNABLE) < 0) {
		user_panic("failed to change child env status.\n");
		return 0;
	}
