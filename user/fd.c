...
fd_alloc(struct Fd **fd)
{
	...
	for (fdno = 0; fdno < MAXFD - 1; fdno++) {
		...
		if (syscall_va_perm(va) == 0) {	//the fd is not used
			*fd = (struct Fd *)va;
			return 0;
		}
	}
...
fd_lookup(int fdnum, struct Fd **fd)
{
	...
	if (syscall_va_perm(va) & PTE_V) {	//the fd is used
		*fd = (struct Fd *)va;
		return 0;
	}
...
dup(int oldfdnum, int newfdnum)
{
	...
	for (i = 0; i < PDMAP; i += BY2PG) {
		int perm = syscall_va_perm(ova + i);
		if(perm < 0) {
			r = perm;
			goto err;
		}
		if (perm > 0 && (perm & PTE_V) == PTE_V) {
			// should be no error here -- pd is already allocated
			if ((r = syscall_mem_map(0, ova + i, 0, nva + i, perm)) < 0) {
				goto err;
			}
		}
	}

	if ((r = syscall_mem_map(0, (u_int)oldfd, 0, (u_int)newfd, syscall_va_perm(oldfd))) < 0) {
		goto err;
	}
...
read(int fdnum, void *buf, u_int n)
{
	...
	if((r = fd_lookup(fdnum, &fd)) < 0) {
		printf("fd.c: <read> failed to lookup fd %d\n", fdnum);
		return r;
	}
	if((r = dev_lookup(fd->fd_dev_id, &dev)) < 0) {
		printf("fd.c: <read> failed to lookup dev %d\n", fd->fd_dev_id);
		return r;
	}
	...
	if((r = (*dev->dev_read)(fd, buf, n, fd->fd_offset)) < 0) {
		printf("fd.c: <read> dev read failed r(%d)\n", r);
		return r;
	}
