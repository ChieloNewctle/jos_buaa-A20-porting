...
u_char fsipcbuf[BY2PG] __attribute__((aligned(16 * 1024)));		// page-aligned, declared in entry.S
...
fsipc(u_int type, void *fsreq, u_int dstva, u_int *perm)
{
	...
	ipc_send(FS_SERV_ENVID, type, (u_int)fsreq, PTE_V | PTE_R);
...
fsipc_map(u_int fileid, u_int offset, u_int dstva)
{
	...
	if ((perm & PTE_V) != (PTE_V)) {
		panic("fsipc_map: unexpected permissions %08x for dstva %08x", perm,
				   dstva);
	}
