...
//	are memory-mapped, this amounts to a bcopy() surrounded by a little red
...
ftruncate(int fdnum, u_int size)
{
	...
	for (i = ROUND(size, BY2PG); i < ROUND(oldsize, BY2PG); i += BY2PG)
		if ((r = syscall_mem_unmap(0, va + i)) < 0) {
			panic("ftruncate: syscall_mem_unmap %08x: %e", va + i, r);
		}
